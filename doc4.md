...
**Functionality**:
- **Data Retrieval**: Fetches classroom reservations from the `requests` table for the next day.
- **Data Processing**: Processes the retrieved data to generate a daily report.
- **Email Notification**: Sends the generated report to the administrative office via email.
- **Error Handling**: Implements error handling for:
    - Database query errors (PDOException).
    - Email sending issues.
...
**Output**:
- No output directly produced, but the script generates and emails a daily report of classroom reservations for the next day to the administrative office.
...
**Functionality**:
- **Classroom Submission**: Allows administrators to add new classrooms to the `classrooms` table, making them available for booking.
- **Data Validation**: Validates and sanitizes user inputs to prevent common web vulnerabilities and ensure data integrity.
- **Database Insertion**: Inserts the new classroom data into the `classrooms` table.
- **Error Handling**: Implements error handling to catch and manage exceptions, providing informative JSON responses in case of invalid requests or server errors.
...
- POST request with form data containing:
    ...
    - `campus` (required): Campus where the classroom is located.
    - `capacity` (required): Maximum capacity of the classroom.
    - `type` (required): Type of the classroom (e.g., "Lecture Hall", "Computer Lab").
    - `status` (required): Current status of the classroom (e.g., "Available", "Unavailable").
**Description**:  API endpoint for submitting clues for lost and found items.
...
**Functionality**:
- **Lost and Found Item Submission**: Handles user submissions for lost and found items, allowing users to post requests for items they have either lost or found.
- **Data Validation**: Validates and sanitizes user-provided data, ensuring required fields are present and data is safe for database insertion.
- **Database Insertion**: Inserts the lost and found item details into the `lost_and_found` table, including item type (lost or found), student name, details, location, email, campus, file path, reward (if any), alternate contact, and a password for managing the entry.
- **Password Hashing**: Securely hashes the provided password before storing it in the database.
- **Confirmation Email**: Sends a confirmation email to the user upon successful submission, including a unique entry ID and a passphrase for managing their submission.
- **Error Handling**: Implements error handling for invalid request methods, data validation failures, database operation errors, and email sending issues.
- **Logging**: Utilizes a logging mechanism (`lf_logger`) to record lost and found item submissions for auditing and tracking.

**Requires**:
- **PHP with PDO Extension**: For database interactions.
- **PHPMailer Library**: For sending confirmation emails.
- **Global Configuration Files**: 
    - `db.php`: For database connection configuration.
    - `global_variables.php`: For global settings, including email server configurations and security keys.
    - `global_error_handler.php`: For centralized error logging and handling.

** входные данные**:
- POST request with form data containing:
    - `type` (required): Type of submission, either 'lost' or 'found'.
    - `student_name` (required): Name of the student submitting the request.
    - `detail` (required): Detailed description of the lost or found item.
    - `location` (required): Location where the item was lost or found.
    - `email` (required): Email address of the submitter.
    - `campus` (required): Campus where the item was lost or found.
    - `file_path` (required): Path to any uploaded file related to the item.
    - `password` (required): Passphrase for managing the submission.
    - `event_time` (required): Time of the event (when the item was lost or found).
    - `reward` (optional): Reward offered for lost items (if applicable).
    - `alt_contact` (optional): Alternative contact information.

**Output**:
- **JSON Response**:
    - `success: true`: If the lost and found entry is successfully submitted and email notification is sent (or email sending fails but entry submission is successful).
    - `success: false`: If there are any errors during the submission process, accompanied by an error `message`.
    - `message`: A descriptive message indicating the outcome, including the new L&F entry ID on success, or error details on failure.
...
**Functionality**:
- **Repair Request Submission**: Allows users to submit repair requests for various issues, capturing details such as student name, subject, description, location, and contact email.
- **Data Validation and Sanitization**: Validates and sanitizes user inputs to prevent common web vulnerabilities and ensure data integrity.
- **Database Insertion**: Inserts the repair request data into the `repair_requests` table, including student information, repair details, and submission timestamp.
- **Error Handling**: Implements error handling to catch and manage exceptions, providing informative JSON responses in case of invalid requests or server errors.
...
- POST request with form data:
    - `studentName` (required): Name of the student submitting the repair request.
    - `subject` (required): Subject of the repair request.
    - `detail` (required): Detailed description of the repair issue.
    - `location` (required): Location of the issue.
    - `email` (required): Contact email of the student.
    - `campus` (required): Campus where the issue is located.
    - `filePath` (required): Path to uploaded file, if any (e.g., image of the issue).
    - `token` (required): Admin authentication token (although admin authentication is not enforced in this endpoint, token validation is still performed).

**Output**:
- **JSON Response**:
    - `success: true`: Indicates successful submission of the repair request.
    - `success: false`: Indicates a failure in submitting the repair request.
    - `message`: A descriptive message providing additional information about the success or failure.
    - `error`: (optional) Detailed error message in case of failure.
