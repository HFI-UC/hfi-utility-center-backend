...
{
    global $pdo;
    if($_SERVER['HTTP_CF_CONNECTING_IP']=='120.24.212.59'||$_SERVER['HTTP_CF_CONNECTING_IP']=='154.37.213.151'){
        exit;
    }
    try {
        $stmt = $pdo->prepare("
...
function sendErrorResponse($statusCode, $message)
{
    ...
    echo json_encode([
        'message' => $message . ' If the error persists, please contact feedback@hfiuc.org and attach the timestamp.',
        'timestamp' => date('Y-m-d H:i:s')
    ], JSON_UNESCAPED_UNICODE);

    // 设置一个全局标志，防止在 shutdownFunction 中重复记录
...
function customErrorHandler($errno, $errstr, $errfile, $errline)
{
    ...
    header('Content-Type: application/json; charset=UTF-8');
    echo json_encode([
        'message' => 'Internal Server Error.'.' If the error persists, please contact feedback@hfiuc.org and attach the timestamp.',
        'timestamp' => date('Y-m-d H:i:s')
    ], JSON_UNESCAPED_UNICODE);
